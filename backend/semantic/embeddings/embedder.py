...
def _get_model():
    ...
    with _model_lock:
        ...
        try:
            ...
            # Warm-up
            _model.encode("test", convert_to_numpy=True)
...
def encode(texts: List[str], batch_size: int = 32) -> Optional[np.ndarray]:
    ...
    Returns:
        Matrice numpy de shape (len(texts), DIMENSION) ou None
    """
    model = _get_model()
    if model is None:
        return None
...
def encode_with_variants(text: str, variants: List[str]) -> Optional[np.ndarray]:
    """
    Encode un texte avec ses variantes et retourne la moyenne.
    ...
    Returns:
        Vecteur moyen normalisé ou None
    """
...
def build_faiss_index(
    ...
    Args:
        ...
        item_texts: Liste des textes normalisés (libelle_recherche)
        item_labels: Liste des libellés canoniques

    Returns:
        Dictionnaire contenant l'index FAISS ou None
    """
    ...
    embeddings = encode(item_texts, batch_size=32)
...
def get_or_build_index(
    ...
    Cache : 10 minutes, nettoyage automatique des entrées > 1 heure.
...
def search_embedding(
    ...
    """
    Recherche les items les plus similaires par similarité cosinus.

    Args:
        query: Texte de la requête
        variants: Variantes synonymiques
        index_obj: Index FAISS
        top_k: Nombre de résultats

    Returns:
        Liste de candidats [{item_id, label, score, rank}, ...]
    """
...
def invalidate_cache(concession_id: Optional[int] = None) -> None:
    ...
    Args:
        concession_id: ID de la concession à invalider (None = tout)
    """
    global _index_cache
    with _cache_lock:
