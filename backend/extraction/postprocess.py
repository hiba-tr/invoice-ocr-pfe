...
def find_header_end(matrix: List[List[str]], num_rows: int) -> int:
    ...
    # Forcer au moins 1 ligne de header si la ligne 0 ressemble à des labels
    row0_text = " ".join(str(c or "").lower() for c in matrix[0])
    row0_score = sum(1 for sig in HEADER_SIGNALS if sig in row0_text)
    row0_non_empty = [str(c or "").strip() for c in matrix[0] if str(c or "").strip()]
    row0_numeric = sum(1 for v in row0_non_empty if is_numeric(v))
    
    # Si la ligne 0 a des signaux header OU est majoritairement textuelle → c'est un header
    if row0_score > 0 or (row0_non_empty and row0_numeric / len(row0_non_empty) < 0.5):
        # Chercher la fin du header à partir de la ligne 1
        for r in range(1, min(num_rows, 5)):
            ...
            # Ligne de données : majoritairement numérique ET pas de signal header
            if score == 0 and numeric_count / len(non_empty) > 0.5:
                return r
        return 1  # Par défaut : 1 ligne de header
    
    return 1  # Sécurité : toujours au moins 1 ligne de header


def extract_col_names(matrix, header_end, num_cols):
    # Sécurité : si header_end=0, utiliser quand même la ligne 0
    effective_end = max(header_end, 1)
    col_names = []
    for ci in range(num_cols):
        parts = []
        for ri in range(effective_end):
