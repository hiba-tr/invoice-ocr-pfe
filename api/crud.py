...
def create_fact_data(
    ...
    """
    items_data = [{"description": "...", "valeurs": {"colonne1": val, ...}}]
    Commit global à la fin (une seule transaction).
    """
    added = False
    for item_dict in items_data:
        ...
        for col_nom, valeur in item_dict.get("valeurs", {}).items():
            ...
            existing = db.query(models_sql.FactData).filter(
                ...
                models_sql.FactData.id_item == item.id_item,
                models_sql.FactData.id_colonne == colonne.id_colonne,
            ).first()
            if not existing:
    ...
    db.commit()  # commit systématique (idempotent même si rien n'a changé)
...
def create_fact_data(
    ...
    """
    Insère les données en évitant les doublons (combinaison id_item, id_colonne) au sein du même lot.
    """
    ...
    for item_dict in items_data:
        ...
        for col_nom, valeur in item_dict.get("valeurs", {}).items():
            ...
            # Si la même combinaison apparaît plusieurs fois, on garde la dernière valeur
